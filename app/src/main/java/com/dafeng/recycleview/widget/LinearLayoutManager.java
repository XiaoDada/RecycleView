...
package com.dafeng.recycleview.widget;
...
public class LinearLayoutManager extends RecyclerView.LayoutManager implements
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        //mAnchorInfo 锚点  mValid默认为false，一次测量后设为true，onLayout完成后重新设置为false
        if (!mAnchorInfo.mValid || mPendingScrollPosition != RecyclerView.NO_POSITION
                || mPendingSavedState != null) {
            mAnchorInfo.reset();
            //^ 位运算符  异或  参加运算的两个对象，如果两个相应位为“异”（值不同），则该位结果为1，否则为0。
...
//    / * *
//            *神奇功能:)。
//    填充由layoutState定义的给定布局。
//    这是相当
//*独立于{@link LinearLayoutManager}的其余部分
//*和很少的变化，可以公开提供作为助手类。
//            *
//            * @param回收器当前的回收器附属于回收视图
//* @param layoutState配置关于我们应该如何填写可用空间。
//            *通过RecyclerView来控制滚动步骤的@param状态上下文。
//            * @param stopOnFocusable。如果为真，填充停止在第一个可调焦的新子元素中
//* @返回它添加的像素数。
//    对滚动函数很有用。
//            * /
